...
package org.dromara.dante.message.servlet.websocket.sender;
...
public class MultipleInstanceMessageSyncConsumer implements Consumer<StompWebSocketMessage> {
    ...
    @Override
    public void accept(StompWebSocketMessage webSocketMessage) {
        if (Strings.CS.equals(webSocketMessage.getUserId(), MessageConstants.MESSAGE_TO_ALL)) {
            webSocketMessagingTemplate.broadcast(webSocketMessage.getDestination(), webSocketMessage.getPayload());
        } else {
            webSocketMessagingTemplate.pointToPoint(webSocketMessage.getUserId(), webSocketMessage.getDestination(), webSocketMessage.getPayload());
        }
    }
}
