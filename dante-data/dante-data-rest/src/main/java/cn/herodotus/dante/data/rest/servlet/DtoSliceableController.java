 ...
 */
public interface DtoSliceableController<O extends BaseDomain, E extends BaseEntity, ID extends Serializable, S extends BaseSliceableService<E, ID>> extends DtoReadableController<O, E, ID, S> {
    ...
    default Result<Map<String, Object>> findByPage(Integer pageNumber, Integer pageSize) {
        Slice<E> slice = getService().findByPage(pageNumber, pageSize);
        Slice<O> response = slice.map(item -> getToDtoConverter().convert(item));
        return resultFromSlice(response);
    }
    ...
    default Result<Map<String, Object>> findByPage(Integer pageNumber, Integer pageSize, Sort.Direction direction, String... properties) {
        Slice<E> slice = getService().findByPage(pageNumber, pageSize, direction, properties);
        Slice<O> response = slice.map(item -> getToDtoConverter().convert(item));
        return resultFromSlice(response);
    }
}
