...
import cn.herodotus.dante.data.commons.service.BaseSliceableService;
...
/**
 * <p>Description: 支持 {@link Slice} 类型分页的基础 Controller  “读” 抽象定义 </p>
 * <p>
...
public abstract class AbstractDtoSliceableController<O extends BaseDomain, E extends BaseEntity, ID extends Serializable, S extends BaseSliceableService<E, ID>> implements DtoSliceableController<O, E, ID, S> {
    ...
    public Result<Map<String, Object>> findByPage(@Validated Pager pager) {
        if (ArrayUtils.isNotEmpty(pager.getProperties())) {
            ...
            return DtoSliceableController.super.findByPage(pager.getPageNumber(), pager.getPageSize(), direction, pager.getProperties());
        } else {
            return DtoSliceableController.super.findByPage(pager.getPageNumber(), pager.getPageSize());
        }
    }
}
