 ...
 */
public abstract class AbstractEntityWriteAndPageController<E extends BaseEntity, ID extends Serializable, S extends BaseWriteAndPageService<E, ID>> extends AbstractEntityPageableController<E, ID, S> implements EntityWriteAndPageController<E, ID, S> {
    ...
    @PostMapping
    @Override
    public Result<E> save(@Validated @RequestBody E domain) {
        return EntityWriteAndPageController.super.save(domain);
    }
    ...
    public Result<String> delete(@PathVariable ID id) {
        return EntityWriteAndPageController.super.delete(id);
    }
}
