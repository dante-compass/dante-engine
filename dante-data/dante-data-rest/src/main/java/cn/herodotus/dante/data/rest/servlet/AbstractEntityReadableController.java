...
public abstract class AbstractEntityReadableController<E extends BaseEntity, ID extends Serializable, S extends BasePageableService<E, ID>> implements EntityPageableController<E, ID, S> {
    ...
    public Result<Map<String, Object>> findByPage(@Validated Pager pager) {
        if (ArrayUtils.isNotEmpty(pager.getProperties())) {
            ...
            return EntityPageableController.super.findByPage(pager.getPageNumber(), pager.getPageSize(), direction, pager.getProperties());
        } else {
            return EntityPageableController.super.findByPage(pager.getPageNumber(), pager.getPageSize());
        }
    }
}
